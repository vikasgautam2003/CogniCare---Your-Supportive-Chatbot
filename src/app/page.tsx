...
//     const handleLoad = () => {
//       setTimeout(() => setLoading(false), 2000); // small delay for smooth fade-out
//     };
...
export default function Page() {
  ...
  useEffect(() => {
    const handleLoad = () => {
      // Ensure smooth fade even after video is ready
      setTimeout(() => setLoading(false), 800); 
    };

    // Find background video(s)
    ...
    if (videos.length > 0) {
      ...
      videos.forEach((video) => {
        if (video.readyState >= 3) {
          ...
          loadedCount++;
          if (loadedCount === videos.length) handleLoad();
        } else {
